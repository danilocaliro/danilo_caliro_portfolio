...
const workCases = [
  ...
  },
  {
    ...
    previewImage: "/assets/placeholders/previews/preview_commercial_01.webp?v=5cee40b",
    galleryImages: ["/assets/placeholders/previews/preview_commercial_01.webp?v=5cee40b", "/assets/placeholders/previews/preview_commercial_02.webp", "/assets/placeholders/previews/preview_commercial_03.webp", "/assets/placeholders/previews/preview_commercial_04.webp"],
  ...
  },
  {
    ...
    previewImage: "/assets/placeholders/previews/preview_cultural_01.webp?v=5cee40b",
    galleryImages: ["/assets/placeholders/previews/preview_cultural_01.webp?v=5cee40b", "/assets/placeholders/previews/preview_cultural_02.webp?v=5cee40b", "/assets/placeholders/previews/preview_cultural_03.webp", "/assets/placeholders/previews/preview_cultural_04.webp"],
...
function SectionEndArrow({ href, label, direction = "down" }: { href: string; label: string; direction?: "up" | "down" }) {
...
export default function Home() {
  ...
  const [mobileImagePan, setMobileImagePan] = useState(0);
  const [mobileImagePanY, setMobileImagePanY] = useState(0);
  const [desktopImageScale, setDesktopImageScale] = useState(1);
  const [desktopImagePan, setDesktopImagePan] = useState({ x: 0, y: 0 });
  const desktopDragStart = useRef<{ x: number; y: number; panX: number; panY: number } | null>(null);
  ...
  return (
    <div className="site-shell">
      ...
      <main id="main">
        ...
        {expandedWorkImage && (() => { const activeWork = workCases.find((work) => work.id === activeWorkId) ?? workCases[0]; const gallery = activeWork.galleryImages; return <div className="work-image-lightbox" role="dialog" aria-modal="true" aria-label={`${activeWork.category} image gallery`} onClick={() => setExpandedWorkImage(false)}><div className="work-image-lightbox-stage" onClick={(event) => { if (event.target === event.currentTarget) setExpandedWorkImage(false); }}><button type="button" className="work-image-lightbox-arrow work-image-lightbox-prev" onClick={(event) => { event.stopPropagation(); setGalleryIndex((index) => (index - 1 + gallery.length) % gallery.length); }} aria-label="Previous image"><ChevronLeft size={28} /></button><div className="work-image-lightbox-image" role="img" aria-label={`${activeWork.category} image ${galleryIndex + 1} of ${gallery.length}`} onClick={(event) => { event.stopPropagation(); if (!window.matchMedia("(max-width: 700px)").matches) { return; } const now = Date.now(); if (now - lastTapTime.current < 320) { const nextScale = mobileImageScale > 1 ? 1 : 2; setMobileImageScale(nextScale); setMobileImagePan(0); setMobileImagePanY(0); } lastTapTime.current = now; }} onDoubleClick={(event) => { event.stopPropagation(); if (!window.matchMedia("(max-width: 700px)").matches) setExpandedWorkImage(false); }} onMouseDown={(event) => { if (window.matchMedia("(max-width: 700px)").matches || desktopImageScale <= 1) return; event.preventDefault(); desktopDragStart.current = { x: event.clientX, y: event.clientY, panX: desktopImagePan.x, panY: desktopImagePan.y }; }} onMouseMove={(event) => { const start = desktopDragStart.current; if (!start || desktopImageScale <= 1) return; setDesktopImagePan({ x: Math.max(-42, Math.min(42, start.panX + ((event.clientX - start.x) / Math.max(1, window.innerWidth)) * 100)), y: Math.max(-42, Math.min(42, start.panY + ((event.clientY - start.y) / Math.max(1, window.innerHeight)) * 100)) }); }} onMouseUp={() => { desktopDragStart.current = null; }} onMouseLeave={() => { desktopDragStart.current = null; }} onTouchStart={(event) => { event.stopPropagation(); if (event.touches.length >= 2) { const [first, second] = [event.touches[0], event.touches[1]]; pinchStartDistance.current = Math.hypot(second.clientX - first.clientX, second.clientY - first.clientY); pinchStartScale.current = mobileImageScale; } else { panStartX.current = event.touches[0]?.clientX ?? null; panStartY.current = event.touches[0]?.clientY ?? null; panStartOffset.current = mobileImagePan; panStartOffsetY.current = mobileImagePanY; } }} onTouchMove={(event) => { event.stopPropagation(); if (event.touches.length >= 2 && pinchStartDistance.current !== null) { const [first, second] = [event.touches[0], event.touches[1]]; const distance = Math.hypot(second.clientX - first.clientX, second.clientY - first.clientY); const nextScale = Math.min(3.45, Math.max(1, pinchStartScale.current * distance / pinchStartDistance.current)); setMobileImageScale(nextScale); if (nextScale === 1) setMobileImagePan(0); return; } if (mobileImageScale > 1 && panStartX.current !== null) { const deltaX = event.touches[0].clientX - panStartX.current; const deltaY = event.touches[0].clientY - (panStartY.current ?? event.touches[0].clientY); setMobileImagePan(Math.max(-42, Math.min(42, panStartOffset.current - (deltaX / Math.max(1, window.innerWidth)) * 100))); setMobileImagePanY(Math.max(-42, Math.min(42, panStartOffsetY.current - (deltaY / Math.max(1, window.innerHeight)) * 100))); } }} onTouchEnd={(event) => { event.stopPropagation(); if (event.touches.length < 2) pinchStartDistance.current = null; const startX = panStartX.current; const endX = event.changedTouches[0]?.clientX ?? startX; panStartX.current = null; panStartY.current = null; if (mobileImageScale > 1.04 || pinchStartScale.current > 1.04) return; if (startX === null || endX === null) return; const distance = endX - startX; if (Math.abs(distance) < 48) return; touchDidSwipe.current = true; if (distance < 0) setGalleryIndex((index) => (index + 1) % gallery.length); else setGalleryIndex((index) => (index - 1 + gallery.length) % gallery.length); }} style={{ backgroundImage: `url(${gallery[galleryIndex]})`, backgroundSize: `${(typeof window !== "undefined" && window.matchMedia("(max-width: 700px)").matches ? mobileImageScale : desktopImageScale) * 100}% auto`, backgroundPosition: `calc(50% + ${(typeof window !== "undefined" && window.matchMedia("(max-width: 700px)").matches ? mobileImagePan : desktopImagePan.x)}%) calc(50% + ${(typeof window !== "undefined" && window.matchMedia("(max-width: 700px)").matches ? mobileImagePanY : desktopImagePan.y)}%)`, touchAction: "none" }} /><button type="button" className="work-image-lightbox-arrow work-image-lightbox-next" onClick={(event) => { event.stopPropagation(); setGalleryIndex((index) => (index + 1) % gallery.length); }} aria-label="Next image"><ChevronRight size={28} /></button><span className="work-image-lightbox-counter">{galleryIndex + 1} / {gallery.length}</span></div></div>; })()}


        <section className="contact-section section-pad" id="contact"><div className="container contact-inner"><SectionLabel>05 / GET IN TOUCH</SectionLabel><h2>Let&apos;s turn ideas<br /><em>into images.</em></h2><p>For production, technical direction, workflow development or tools.</p>{formSent ? <div className="form-success"><Check size={18} /> Thanks — your message has been sent.</div> : <form className="contact-form" action={contactFormEndpoint} method="POST" noValidate onSubmit={async (event) => { event.preventDefault(); const form = event.currentTarget; setFormError(""); if (!form.checkValidity()) { setFormError("Please complete all required fields with a valid email address."); return; } setFormSending(true); try { const response = await fetch(contactFormEndpoint, { method: "POST", body: new FormData(form), headers: { Accept: "application/json" } }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result?.errors?.[0]?.message || "Form submission failed"); setFormSent(true); } catch (error) { setFormError(error instanceof Error ? error.message : "Something went wrong. Please try again in a moment."); } finally { setFormSending(false); } }}><input className="form-honey" type="checkbox" name="_gotcha" tabIndex={-1} autoComplete="off" /><input type="hidden" name="_subject" value="New message from Danilo Caliro Portfolio" /><div className="form-row"><label><span>Your name</span><input type="text" name="name" placeholder="Name" required /></label><label><span>Email</span><input type="email" name="email" placeholder="you@example.com" required /></label></div><label><span>Message</span><textarea name="message" placeholder="Tell me about the project..." rows={4} required /></label>{formError && <p className="form-error" role="alert">{formError}</p>}<button className="button button-dark" type="submit" disabled={formSending}>{formSending ? "Sending…" : "Send message"} {!formSending && <ArrowUpRight size={17} />}</button><small>Your message will be sent securely through Formspree.</small></form>}<SectionEndArrow href="#main" label="Back to Main" direction="up" /></div></section>
      </main>
