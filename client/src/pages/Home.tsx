  ...
  return (
    <div className="site-shell">
      ...
      <main id="main">
        ...
        {expandedWorkImage && (() => { const activeWork = workCases.find((work) => work.id === activeWorkId) ?? workCases[0]; const gallery = activeWork.galleryImages; return <div className="work-image-lightbox" role="dialog" aria-modal="true" aria-label={`${activeWork.category} image gallery`} onClick={() => setExpandedWorkImage(false)}><div className="work-image-lightbox-stage" onClick={(event) => event.stopPropagation()}><button type="button" className="work-image-lightbox-arrow work-image-lightbox-prev" onClick={(event) => { event.stopPropagation(); setGalleryIndex((index) => (index - 1 + gallery.length) % gallery.length); }} aria-label="Previous image"><ChevronLeft size={28} /></button><div className="work-image-lightbox-image" role="img" aria-label={`${activeWork.category} image ${galleryIndex + 1} of ${gallery.length}`} onClick={(event) => { event.stopPropagation(); if (!window.matchMedia("(max-width: 700px)").matches) { setExpandedWorkImage(false); return; } const now = Date.now(); if (now - lastTapTime.current < 320) { const nextScale = mobileImageScale > 1 ? 1 : 2; setMobileImageScale(nextScale); setMobileImagePan(0); } lastTapTime.current = now; }} onTouchStart={(event) => { event.stopPropagation(); if (event.touches.length >= 2) { const [first, second] = [event.touches[0], event.touches[1]]; pinchStartDistance.current = Math.hypot(second.clientX - first.clientX, second.clientY - first.clientY); pinchStartScale.current = mobileImageScale; } else { panStartX.current = event.touches[0]?.clientX ?? null; panStartOffset.current = mobileImagePan; } }} onTouchMove={(event) => { event.stopPropagation(); if (event.touches.length >= 2 && pinchStartDistance.current !== null) { const [first, second] = [event.touches[0], event.touches[1]]; const distance = Math.hypot(second.clientX - first.clientX, second.clientY - first.clientY); const nextScale = Math.min(3, Math.max(1, pinchStartScale.current * distance / pinchStartDistance.current)); setMobileImageScale(nextScale); if (nextScale === 1) setMobileImagePan(0); return; } if (mobileImageScale > 1 && panStartX.current !== null) { const delta = event.touches[0].clientX - panStartX.current; setMobileImagePan(Math.max(-42, Math.min(42, panStartOffset.current + (delta / Math.max(1, window.innerWidth)) * 100))); } }} onTouchEnd={(event) => { event.stopPropagation(); if (event.touches.length < 2) pinchStartDistance.current = null; const startX = panStartX.current; const endX = event.changedTouches[0]?.clientX ?? startX; panStartX.current = null; if (mobileImageScale > 1.04 || pinchStartScale.current > 1.04) return; if (startX === null || endX === null) return; const distance = endX - startX; if (Math.abs(distance) < 48) return; touchDidSwipe.current = true; if (distance < 0) setGalleryIndex((index) => (index + 1) % gallery.length); else setGalleryIndex((index) => (index - 1 + gallery.length) % gallery.length); }} style={{ backgroundImage: `url(${gallery[galleryIndex]})`, backgroundSize: `${mobileImageScale * 100}% auto`, backgroundPosition: `calc(50% + ${mobileImagePan}%) center`, touchAction: "none" }} /><button type="button" className="work-image-lightbox-arrow work-image-lightbox-next" onClick={(event) => { event.stopPropagation(); setGalleryIndex((index) => (index + 1) % gallery.length); }} aria-label="Next image"><ChevronRight size={28} /></button><span className="work-image-lightbox-counter">{galleryIndex + 1} / {gallery.length}</span></div></div>; })()}
